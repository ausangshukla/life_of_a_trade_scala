...
object OrderGen {
  def gen(exchange: String = oneOf(Exchange.NASDAQ, Exchange.NYSE).sample.get,
          ...
          market: String = oneOf(OrderType.MARKET, OrderType.LIMIT).sample.get, 
    ...
    Order(None, exchange, buy_sell, market, user_id, security_id, quantity, unfilled_qty, price, None, None)
  }
}