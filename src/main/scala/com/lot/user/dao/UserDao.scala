...
object UserDao extends TableQuery(new UserTable(_)) {
   ...
   */
  def addBlockedAmount(user_id: Long, amount: Double): Future[Int] = {
    val q = for {
      /*
       * Get the user
       */
   ...
   */
  def deductBlockedAmount(user_id: Long, amount: Double) = {
    val q = for {
      user <- this.filter(_.id === user_id).result.headOption
      blocked_amount: Option[Double] = user.map { _.blocked_amount - amount }
      update <- this.filter(_.id === user_id).map { x => (x.blocked_amount) }.update((blocked_amount.get))
    } yield (update)
