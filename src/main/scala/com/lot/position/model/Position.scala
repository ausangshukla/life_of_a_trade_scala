...
case class Position(id: Option[Long],
    ticker: String,  security_id: Long,  quantity: Double,  average_price: Double,  pnl: Double, 
...
class PositionTable(tag: Tag) extends Table[Position](tag, "positions") {
  ...
  def * = (id.?,  ticker,  security_id,  quantity,  average_price,  pnl,  created_at.?, updated_at.?) <> (Position.tupled, Position.unapply)
}

object PositionJsonProtocol extends CustomJson {
  implicit val positionFormat = jsonFormat8(Position)
}
