...
case class Trade(id: Option[Long],
                 ...
                 security_id: Long, ticker: String, 
                 quantity: Double, price: Double, buy_sell: String,
...
class TradeTable(tag: Tag) extends Table[Trade](tag, "trades") {
  ...
  def state = column[String]("state",  O.Length(15,varying=true))
  ...
  def * = (id.?, trade_date, settlement_date, security_id, ticker, quantity, price, buy_sell, 
...
object TradeJsonProtocol extends CustomJson {
  implicit val tradeFormat = jsonFormat18(Trade)
}
